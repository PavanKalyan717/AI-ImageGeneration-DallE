...
const CreatePost = () => {
  ...
  const handleSubmit =async(e)=>{
    ...
    if(form.prompt && form.photo){
      setLoading(true)
      try {
        const response = await fetch('https://ai-imagegeneration-dalle.onrender.com/api/v1/post',
          {
            method:'POST',
            headers:{
  ...
  const generateImage =async()=>{
    ...
    if(form.prompt) {
      try {
        ...
        const response = await fetch('https://ai-imagegeneration-dalle.onrender.com/api/v1/dalle',
        {
          method:'POST',
        headers:{
          'Content-Type':'application/json'
    ...
    <section className='max-w-7xl mx-auto'>
      ...
      >
        <div className='flex flex-col gap-5'>
          <FormField labelName='Your Name' type='text' name ='name' placeholder='Enter name'
          value={form.name} handleChange= {handleChange}  />
          <FormField labelName='Prompt' type='text' name ='prompt' placeholder='Enter Prompt'
